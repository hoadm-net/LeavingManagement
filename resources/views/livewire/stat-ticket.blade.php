        ...
        <div class="overflow-x-auto">
            <table class="min-w-full border-collapse bg-white ">
                ...
                <tr>
                    ...
                    <th class="px-6 py-4 text-left font-bold text-black border-b">{{ __("From") }}</th>
                    <th class="px-6 py-4 text-left font-bold text-black border-b">{{ __("To") }}</th>
                ...
                @foreach($tickets as $t)
                    <tr class="odd:bg-gray-100 even:bg-white hover:bg-gray-200">
                        <td class="px-6 py-4 border-b">{{ $t->id }}</td>
                        <td class="px-6 py-4 border-b">{{ $t->full_name }} ({{ $t->department->name }})</td>
                        <td class="px-6 py-4 border-b">{{ date('d-m-Y H:i', strtotime($t->from)) }}</td>
                        <td class="px-6 py-4 border-b">{{ date('d-m-Y H:i', strtotime($t->to)) }}</td>
                        <td class="px-6 py-4 border-b"><span class="capitalize">{{ $t->status }}</span></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
